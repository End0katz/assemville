...
public record NamespacedID(
    ...
    public NamespacedID  {
        if (namespace == null || id == null) {
            if (namespace == null && id == null) {
                ...
                throw new NullPointerException(
                        "Cannot create NamespacedID with null namespace (id: %s)".formatted(id)
                );
            } else {
                throw new NullPointerException(
        ...
        if (!namespace.matches("[a-z_-][a-z0-9_-]*")) {
            throw new RuntimeException("Namespace %s does not follow pattern (a-z_- followed by any number, including 0, of repitiions of a-z0-9_-)".formatted(namespace));
        }
